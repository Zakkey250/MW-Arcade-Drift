...
Brake-initiated arcade drifting for **Need for Speed: Most Wanted (2005)**. Version **0.1.0** includes a smooth drift camera, a small slip indicator, counter-steering visuals and per-car JSON profiles. The handling aims for a Criterion-inspired arcade feel; it does not reproduce another game's physics exactly.
...
- [Core](https://github.com/Zakkey250/MW-Arcade-Drift/releases/latest): required, contains the ASI, defaults and guides. **No EXE** is included.
- Editor: optional second ZIP on the same release, adding a Windows slider UI for Drift and Camera settings. Requires .NET Framework 4.x. The Core works without it.

Close the game and extract Core's `scripts/` folder into the game directory. Preserve existing `MWArcadeDrift.ini` and `scripts/MWArcadeDrift/*.json` profiles on updates. Add Editor separately if desired. Remove a legacy `MWCriterionDrift.asi` before loading the new ASI.
...
The previous alpha.4 build had a user drive session with no conspicuous issue; its log recorded active HUD drawing, camera use and 288 completed drifts without a mod fault. FWD visuals and the disabled gray icon were not established by that session. The 0.1.0 build keeps the same gameplay implementation and updates the version label. See [release notes](RELEASE_NOTES_0.1.0.md).
