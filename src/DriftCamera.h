...
struct DriftCamera {
 ...
 void step(float dt,bool active,float beta){
  ...
  const float aim=active?clamp(beta*tuning.orbitGain,-tuning.orbitMaxRad,tuning.orbitMaxRad):0;
  const float bank=active?clamp(-beta*tuning.rollGain,-tuning.rollMaxRad,tuning.rollMaxRad):0;
  spring(orbit,orbitVelocity,aim,dt,tuning.orbitSpeed,tuning.orbitAccel);
  spring(roll,rollVelocity,bank,dt,tuning.rollSpeed,tuning.rollAccel);
  // Independent unsigned envelope avoids pulling back whenever steering changes sign.
  const float proximity=active?clamp(std::abs(beta)/tuning.zoomSlipRad,0.f,1.f):0;
  spring(zoom,zoomVelocity,proximity,dt,tuning.zoomSpeed,tuning.zoomAccel);
 ...
 bool transform(Vector& eye,Vector& center,Vector& up,Vector forward)const{
  ...
  const float closeDistance=std::min(d,std::max(tuning.minDistance,d*(1-tuning.closeFraction*framing)));
  ...
  if(rightLength>.001f){
   ...
   const float side=clamp(orbit/tuning.sideAngleRad,-1.f,1.f);
   // Aim into the visible road half. The car moves to the opposite screen half.
   center=add(center,add(mul(right,side*tuning.sideOffset*closeDistance*framing),mul(view,tuning.lookAhead*closeDistance*framing)));
  }
  ...
  eye=add(eye,mul(axis,clamp(tuning.eyeHeight-height,-tuning.maxDrop,tuning.maxRise)*framing));
  center=add(center,mul(axis,tuning.targetHeight*framing));
