...
struct Controller {
 ...
 Output step(const Sample& s){
  ...
  if(phase==Phase::Drift){
   ...
   if(aligned||neutralTime>.15f)exitReason=3;
  ...
  const float requested=direction*(.09f+.18f*slide+.14f*handbrakePower)*(1.f-.85f*counter);
  ...
  float speedBlend=clamp((s.speed-120.f/3.6f)/(120.f/3.6f),0.f,1.f);
  speedBlend=speedBlend*speedBlend*(3.f-2.f*speedBlend);
  const float turnGain=1.f+.30f*speedBlend;
  float requestedTurn=signedSteer*std::min(.8f,36.f/s.speed)*turnGain*(1.f+.25f*handbrakePower);
  ...
  const float extra=neutral||relative<-.05f?0.f:turnSign*clamp(std::abs(targetTurn)-turnSign*native,0.f,30.f/s.speed);
  ...
  pathAssist+=clamp(pathTarget-pathAssist,-.9f*s.dt,.9f*s.dt);
  ...
  if(counter==0&&!transferring&&s.gas>.5f&&s.brake<.05f&&s.handbrake<.05f&&!wasHandbrake&&s.speedBeforeStep>0&&std::isfinite(loss)&&loss<20.f*s.dt){
   ...
   const float refund=.98f*std::max(0.f,loss);
   const float recovery=std::min(3.f,2.f*deficit)*s.dt;
   o.deltaSpeed=std::min({18.f*s.dt,(refund+recovery)*blend,deficit});
  }
